...
## Checking assumptions using *check_model()*
```{r}
check_model(model)
```
...
```{r}
ref.grid = ref_grid(model)
group.emm = emmeans(ref.grid, "group")
pairs(group.emm, reverse = FALSE)
```

#### Post-hoc comparison for the second factor:
```{r}
ref.grid = ref_grid(model)
time.emm = emmeans(ref.grid, "time")
pairs(time.emm, reverse = FALSE)
```
...
pairs(time.emm, reverse = FALSE)
```
Complete pairwise comparison in case of an interaction
```{r}
#| label: "Facorial RM ANOVA post-hoc emmeans"
emmeans(object = model, specs = ~group*time)
```
