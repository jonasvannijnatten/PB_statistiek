...
```{r}
#| label: "list packages & versions"
#| echo: false
df = data.frame(package = "R", version=substr(R.version.string,11,str_length(R.version.string)))

for (x in 1:length(reqPacks)) {
  df[x+1,1] = reqPacks[x]
  df[x+1,2] = as.character(packageVersion(reqPacks[x]))
}
kable(df[-length(df),], row.names = FALSE) %>%
...
#| echo: false
#| tbl-cap: 'data.long'
kable(data.long[c(1:5,41:45),], row.names = FALSE) %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = FALSE, position = "left")
```
...
data.wide = pivot_wider(data.long, id_cols = ID, names_from = condition, values_from = score)
```
</details>
...
```{r,echo=FALSE}
#| tbl-cap: "data.wide"
kable(data.wide[1:10,], row.names = FALSE) %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = FALSE, position = "left")
```
...
**T-test**
```{r}
t.test(data.wide$A, data.wide$B, alternative = "two.sided", paired = TRUE)
```

### lmer method
```{r,echo=FALSE}
#| tbl-cap: "data.long"
kable(data.long[c(1:5,31:35),], row.names = FALSE) %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = FALSE, position = "left")
```
...
**Fit the model**
```{r}
model = lmer(formula = score ~ condition+(1|ID), data = data.long)
```
...
#| echo: false
#| tbl-cap: "data.long"
kable(data.long[1:5,], row.names = FALSE) %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = FALSE, position = "left")
```
...
#| echo: false
#| tbl-cap: "data.long"
kable(data.long[c(1:2,21:22,41:42),], row.names = FALSE) %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = FALSE, position = "left")
```
...
```{r}
#| label: "Fact. Ind. ANOVA data table"
#| echo: false
#| tbl-cap: "data.long"
...
```{r}
#| label: "Factorial ANOVA visualization"
cat_plot(model = model,   # which model to plot
         pred = treatment,  # variable to plot on the x-axis
         modx = control,    # categorical variable for which to plot seperate colors
         interval = TRUE,  # logical whether to display a confidence interval
         plot.points = TRUE, # logical whether to plot individual data points
         jitter = 0.2) # add jitter (prevents overlapping data point in plot)
```
...
<details><summary><span style="color:dodgerBlue">Show code for data generation </span></summary>
```{r}
#| label: "Factorial RM ANOVA generate dataset"
#| echo: true 
...
```{r}
#| label: "Factorial RM ANOVA show data.wide"
#| echo: false
#| tbl-cap: "data.wide"
kable(data.wide[1:4,], row.names = FALSE, caption = 'data.wide') %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = FALSE, position = "left")
...
```{r}
#| label: "Factorial RM ANOVA show data.long"
#| echo: false
#| tbl-cap: "data.long"
...
```{r}
#| label: "Factorial RM ANOVA fit model"
...
```{r}
#| label: "Factorial RM ANOVA check assumptions"
check_model(model)
```
...
```{r}
#| label: "Factorial RM ANOVA post-hoc main effect 1"
emmeans(object = model, specs = pairwise ~ group)
```

#### Post-hoc comparison for the second factor:
```{r}
#| label: "Factorial RM ANOVA main effect 2"
...
```{r}
#| label: "Factorial RM ANOVA interaction"
...
```{r}
#| label: "Factorial RM ANOVA interaction subset"
...
Post-hoc comparison for the first factor:
```{r}
#| label: "Factorial RM ANOVA post-hoc pairwise T-test main effect 1"
pairwise.t.test(
  x = data.long$score               # column name of the dependent variable
  ,g = data.long$treatment          # column name of the first within-subjects variable
  , paired = TRUE                   # since it is a paired design, set paired=TRUE
...
Post-hoc comparison for the first factor:
```{r}
#| label: "Factorial RM ANOVA post-hoc pairwise T-test main effect 2"
pairwise.t.test(
  x = data.long$score               # column name of the dependent variable
...
Post-hoc comparison for the interaction effect:
```{r}
#| label: "Factorial RM ANOVA post-hoc t-test interaction effect"
pairwise.t.test(
  x = data.long$score               # column name of the dependent variable
  # merge both factor columns into a combined factor
...
## Visualisation
```{r}
#| label: "Factorial RM ANOVA visualization"
